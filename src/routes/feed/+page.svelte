<script lang="ts">
  ...
  function getOrganizationDetails(orgId: string) {
    return data.organizations.find(o => o.id === orgId) || { name: 'Unknown Organization', logo: '' };
  }
</script>
          ...
          <div class="bg-white shadow-lg rounded-lg p-6">
            <div class="flex items-center justify-between mb-2">
              ...
              {#if getOrganizationDetails(post.organizationId).logo}
                <img 
                  src={getOrganizationDetails(post.organizationId).logo} 
                  alt="{getOrganizationDetails(post.organizationId).name} logo" 
                  class="h-16 w-16 object-contain ml-3" 
                />
              {/if}
            </div>
          ...
          <div class="bg-white shadow-lg rounded-lg p-6">
            <div class="flex items-center justify-between mb-2">
              ...
              {#if getOrganizationDetails(report.organizationId).logo}
                <img 
                  src={getOrganizationDetails(report.organizationId).logo} 
                  alt="{getOrganizationDetails(report.organizationId).name} logo" 
                  class="h-16 w-16 object-contain ml-3" 
                />
              {/if}
            </div>
